...
def on_mouse_down(event): # Update when Mouse clicks
  global lastx, lasty
  lastx = event.widget.winfo_pointerx()
  lasty = event.widget.winfo_pointery()

def on_mouse_move(event): # Update when Mouse Hovers
  global lastx, lasty
...
def SerialMonitorRefresh(): # Refresh Serial monitor
    global Serial
    while expanded:
...
def update_model(render_obj,x,y,z): ## Use this to update render in real time 
...
#Title
Label1=tk.Label(root,text='Controller To Serial',font="play 18 bold",fg="#001122", bg="#dddddd",highlightthickness=0)
Label1.pack()
